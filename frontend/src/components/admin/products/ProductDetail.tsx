...
const ProductDetail: React.FC = () => {
  ...
  const variantColumns: ColumnsType<ProductVariant> = [
      ...
      { title: 'Giá sale', dataIndex: 'salePrice', key: 'salePrice', render: formatPrice },
  ...
  const InfoItem: React.FC<{ label: string; children: React.ReactNode }> = ({ label, children }) => (
    <div className="mb-2">
        ...
        <div className="font-semibold">{children || <Text type="secondary">N/A</Text>}</div>
    </div>
  );
    ...
    <div className="p-6 bg-gray-100 min-h-screen">
      <Card className="bg-white shadow-lg rounded-xl mb-6">
        <Row justify="space-between" align="middle">
          <Col>
            <Title level={3} className="!m-0">Chi tiết sản phẩm</Title>
            <Text type="secondary">{product.name}</Text>
          </Col>
          <Col>
                              <Space>
              <Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/admin/products')}>Quay lại</Button>
              <Button type="primary" icon={<EditOutlined />} onClick={() => navigate(`/admin/products/edit/${product._id}`)}>Chỉnh sửa</Button>
              <Button danger icon={<DeleteOutlined />} onClick={handleSoftDelete}>Xóa</Button>
                              </Space>
                              </Col>
                            </Row>
                  </Card>
        ...
        <Col xs={24} lg={8}>
            <Space direction="vertical" size="large" style={{width: '100%'}}>
                <Card className="bg-white shadow-lg rounded-xl">
                    ...
                    src={mainImage}
                    alt={product.name}
                    style={{ borderRadius: '8px', border: '1px solid #f0f0f0', marginBottom: '16px' }}
                    />
                    <Image.PreviewGroup>
                    <Space wrap>
                        {product.images.map((image, index) => (
                            ...
                            onClick={() => setMainImage(image)}
                                style={{
                            borderRadius: '4px', 
                            border: mainImage === image ? '2px solid #1890ff' : '1px solid #d9d9d9',
                            cursor: 'pointer',
                            objectFit: 'cover'
                            }}
                            preview={{ src: image }}
                ...
                <Card className="bg-white shadow-lg rounded-xl">
                    ...
                    <InfoItem label="Hiển thị">
                        <Tag icon={product.isActive ? <CheckCircleOutlined /> : <CloseCircleOutlined />} color={product.isActive ? 'success' : 'error'}>
                    ...
                    <InfoItem label="SKU">{product.sku}</InfoItem>
                </Card>
            </Space>
        </Col>
            ...
            <Card className="bg-white shadow-lg rounded-xl h-full">
                ...
                <div className="mb-4">
                    ...
                    <Space align="baseline">
                        <Text delete type="secondary" style={{ fontSize: '1.2rem' }}>{formatPrice(product.price)}</Text>
                        <Text type="danger" strong style={{ fontSize: '2rem' }}>{formatPrice(product.salePrice)}</Text>
                        <Tag color="red">-{Math.round(((product.price - product.salePrice) / product.price) * 100)}%</Tag>
                    </Space>
                    ) : (
                    <Text strong style={{ fontSize: '2rem' }}>{formatPrice(product.price)}</Text>
                  )}
                </div>
                <Tabs defaultActiveKey="1">
                    <Tabs.TabPane tab="Tổng quan" key="1">
                        <Row gutter={32}>
                            <Col span={12}>
                                ...
                                <InfoItem label="Danh mục"><Tag color="blue">{categoryName}</Tag></InfoItem>
                                <InfoItem label="Thương hiệu"><Tag color="geekblue">{brandName}</Tag></InfoItem>
                                <InfoItem label="Bảo hành">{product.warranty || 0} tháng</InfoItem>
                                <InfoItem label="Cân nặng">{product.weight ? `${product.weight} gram` : 'N/A'}</InfoItem>
                                <InfoItem label="Kích thước">{product.dimensions ? `${product.dimensions.length || 0} x ${product.dimensions.width || 0} x ${product.dimensions.height || 0} (cm)` : 'N/A'}</InfoItem>
                                <InfoItem label="Tags">{product.tags && product.tags.length > 0 ? product.tags.map(tag => <Tag key={tag}>{tag}</Tag>) : <Text type="secondary">Không có thẻ tag.</Text>}</InfoItem>
                    </Col>
                            <Col span={12}>
                                <InfoItem label="Meta Title">{product.metaTitle}</InfoItem>
                                <InfoItem label="Meta Description">{product.metaDescription}</InfoItem>
                    </Col>
                  </Row>
                        <Divider />
                    ...
                    <Tabs.TabPane tab="Thông số & Tính năng" key="2">
                        <Row gutter={32}>
                            <Col span={12}>
                    ...
                    <Tabs.TabPane tab="Tối ưu SEO" key="4">
                        <InfoItem label="Meta Title">{product.metaTitle}</InfoItem>
                        <InfoItem label="Meta Description">{product.metaDescription}</InfoItem>
                        <InfoItem label="Tags">
