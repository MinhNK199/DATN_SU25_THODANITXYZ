              ...
              >
                ...
                >
                  <h3 className="font-bold text-lg mb-3 text-gray-900">Giỏ hàng</h3>
                    ...
                    <>
                      <div className="max-h-64 overflow-y-auto divide-y divide-gray-100 mb-3 cart-scroll">
                        {cartState.items.slice(0, 3).map(item => (
                            <div key={item._id} className="flex items-center py-2 gap-2">
                              <img src={item.product.images && item.product.images[0] ? item.product.images[0] : '/placeholder-image.jpg'} alt={item.product.name} className="w-12 h-12 object-contain rounded-lg border flex-shrink-0" />
                            <div className="flex-1 min-w-0">
                                <div className="font-medium text-gray-900 line-clamp-1 text-sm">{item.product.name}</div>
                              <div className="text-xs text-gray-500">Số lượng: {item.quantity}</div>
                            </div>
                              <div className="font-semibold text-blue-600 whitespace-nowrap text-sm">{formatPrice(item.product.salePrice || item.product.price)}</div>
                          </div>
                        ))}
                        {cartState.items.length > 3 && (
                          <div className="text-center text-gray-500 py-2 text-sm">... và {cartState.items.length - 3} sản phẩm khác</div>
                        )}
                      </div>
