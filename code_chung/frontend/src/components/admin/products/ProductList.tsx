...
import { getProducts, getDeletedProducts, softDeleteProduct, restoreProduct, hardDeleteProduct, getBrands } from "./api";
import { Card, Table, Typography, Button, Space, Input, Select, Avatar, Tag, Modal, Tooltip, message, Row, Col, Badge } from "antd";
import { PlusOutlined, DeleteOutlined, EyeOutlined, EditOutlined, SearchOutlined, UndoOutlined, ExclamationCircleFilled } from "@ant-design/icons";
...
const ProductListPage: React.FC = () => {
    ...
    const handleHardDelete = (id: string) => {
        confirm({
            ...
            icon: <ExclamationCircleFilled />,
            content: 'Hành động này không thể hoàn tác. Sản phẩm sẽ bị xóa hoàn toàn.',
    ...
    const filteredProducts = products
        ...
        .filter(p => filterBrand ? (typeof p.brand === 'object' ? p.brand._id === filterBrand : p.brand === filterBrand) : true)
        .filter(p => {
    ...
    const columns: ColumnsType<Product> = [
            ...
            render: (_, record) => (
                <Space>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        <Text type="secondary">SKU: {record.sku || 'N/A'}</Text>
                    </div>
                </Space>
            ),
        },
        ...
        },
        {
            ...
            dataIndex: 'category',
            key: 'category',
            render: (category) => typeof category === 'object' ? category.name : 'N/A'
        },
        {
            title: 'Trạng thái',
            key: 'status',
            ...
            render: (_, record) => (
                <Space size="middle">
                    <Tooltip title="Xem chi tiết">
                        <Button icon={<EyeOutlined />} onClick={(e) => { e.stopPropagation(); navigate(`/admin/products/detail/${record._id}`); }} />
                    </Tooltip>
                    <Tooltip title="Chỉnh sửa">
                        <Button icon={<EditOutlined />} onClick={(e) => { e.stopPropagation(); navigate(`/admin/products/edit/${record._id}`); }} />
                    </Tooltip>
                    <Tooltip title="Xóa (Thùng rác)">
                        <Button danger icon={<DeleteOutlined />} onClick={(e) => { e.stopPropagation(); handleSoftDelete(record._id); }} />
                    </Tooltip>
                </Space>
            ),
        },
