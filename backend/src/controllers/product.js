  ...
  try {
    ...
    res.json({
      ...
      stats: {
        ...
        avgRating:
          Array.isArray(stats.avgRating) && stats.avgRating[0] && stats.avgRating[0].avgRating
            ? stats.avgRating[0].avgRating
            : 0,
      },
    })
  } catch (error) {
...
export const createProduct = async (req, res) => {
  try {
    ...
    // Process variants với error handling tốt hơn
    let processedVariants = []
    if (req.body.variants && Array.isArray(req.body.variants)) {
      try {
        processedVariants = req.body.variants.map((variant, index) => {
          try {
            return processVariantData(variant, index)
          } catch (error) {
            throw new Error(`Lỗi xử lý biến thể ${index + 1}: ${error.message}`)
          }
        })
      } catch (error) {
    ...
    const product = new Product({
      ...
      dimensions: {
        length:
          req.body.dimensions && typeof req.body.dimensions.length !== "undefined" ? req.body.dimensions.length : 0,
        width: req.body.dimensions && typeof req.body.dimensions.width !== "undefined" ? req.body.dimensions.width : 0,
        height:
          req.body.dimensions && typeof req.body.dimensions.height !== "undefined" ? req.body.dimensions.height : 0,
      },
    })
    ...
    product.meta = {
      metaTitle:
        typeof metaTitle !== "undefined"
          ? metaTitle
          : product.meta && product.meta.metaTitle
            ? product.meta.metaTitle
            : "",
      metaDescription:
        typeof metaDescription !== "undefined"
          ? metaDescription
          : product.meta && product.meta.metaDescription
            ? product.meta.metaDescription
            : "",
      metaImage:
        typeof metaImage !== "undefined"
          ? metaImage
          : product.meta && product.meta.metaImage
