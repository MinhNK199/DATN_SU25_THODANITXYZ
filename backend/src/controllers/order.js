...
export const updateOrderToPaidCOD = async (req, res) => {
    try {
        ...
        if (order.paymentMethod !== 'COD') {
            return res.status(400).json({ message: "Chỉ cập nhật thanh toán cho đơn hàng COD" });
        }
        if (order.isPaid) {
        ...
        await order.save();
        res.json({ message: "Cập nhật trạng thái thanh toán COD thành công", order });
    } catch (error) {
        res.status(400).json({ message: error.message });
    }
};
...
async function createNotificationForUser(userId, title, message, type = 'order', link = '', data = {}) {
...
export const getValidOrderStatusOptions = async (req, res) => {
    try {
        ...
        if (order.status === 'delivered_success' && !order.isPaid) {
            options = options.filter(opt => opt === 'returned' || opt === 'completed');
        }
        // Chỉ cho phép yêu cầu hoàn tiền nếu trạng thái là completed và trong 3 ngày
        if (order.status === 'completed') {
            // Tìm thời điểm chuyển sang completed
            const completedHistory = order.statusHistory.find(s => s.status === 'completed');
            if (completedHistory) {
                const now = new Date();
...
export const updateOrderStatus = async (req, res) => {
    try {
        ...
        if (!validTransitions.hasOwnProperty(currentStatus)) {
            return res.status(400).json({ message: `Không thể chuyển trạng thái từ ${currentStatus}` });
        }
        if (!validTransitions[currentStatus].includes(status)) {
            // Nếu là refund_requested thì kiểm tra điều kiện đặc biệt
            if (status === 'refund_requested') {
                if (order.status !== 'completed') {
                    return res.status(400).json({ message: 'Chỉ có thể yêu cầu hoàn tiền khi đơn hàng đã hoàn thành.' });
                }
                // Kiểm tra thời gian completed
                const completedHistory = order.statusHistory.find(s => s.status === 'completed');
                if (!completedHistory) {
                    return res.status(400).json({ message: 'Không tìm thấy thời điểm hoàn thành đơn hàng.' });
                }
                const now = new Date();
                const completedAt = new Date(completedHistory.date);
                const diffDays = (now - completedAt) / (1000 * 60 * 60 * 24);
                if (diffDays > 3) {
                    return res.status(400).json({ message: 'Chỉ có thể yêu cầu hoàn tiền trong vòng 3 ngày kể từ khi đơn hàng hoàn thành.' });
                }
            } else {
                return res.status(400).json({ message: `Chuyển trạng thái từ ${currentStatus} sang ${status} không hợp lệ!` });
            }
        }
        ...
        if (status === 'completed') {
            if (order.status !== 'delivered_success') {
                return res.status(400).json({ message: 'Chỉ có thể chuyển sang trạng thái Thành công khi đơn hàng đã giao thành công.' });
            }
            if (!order.isPaid) {
                return res.status(400).json({ message: 'Chỉ có thể chuyển sang trạng thái Thành công khi đơn hàng đã được thanh toán.' });
            }
        }

        // Đếm số lượt yêu cầu hoàn tiền
        const refundCount = order.statusHistory.filter(s => s.status === 'refund_requested').length;
        const maxRefund = 3;
        ...
        if (order.status === 'refunded') {
          await createNotificationForUser(order.user, 'Hoàn tiền thành công', `Yêu cầu hoàn tiền cho đơn hàng #${order._id} đã được chấp nhận và xử lý thành công.`, 'order', `/profile?tab=orders`, { orderId: order._id });
        }
        if (order.status === 'delivered_success' && refundCount > 0) {
          await createNotificationForUser(order.user, 'Từ chối hoàn tiền', `Yêu cầu hoàn tiền cho đơn hàng #${order._id} đã bị từ chối. Bạn còn ${remain} lần yêu cầu hoàn tiền.`, 'order', `/profile?tab=orders`, { orderId: order._id, remain });
        }
        res.json(updatedOrder);
    } catch (error) {
        res.status(400).json({ message: error.message });
...
export const requestRefund = async (req, res) => {
    try {
        ...
        if (!order) return res.status(404).json({ message: "Không tìm thấy đơn hàng" });
        if (order.status !== 'delivered_success') {
            return res.status(400).json({ message: 'Chỉ có thể yêu cầu hoàn tiền khi đơn hàng đã giao thành công.' });
        }
        if (!order.isPaid) {
            return res.status(400).json({ message: 'Chỉ có thể yêu cầu hoàn tiền khi đơn hàng đã được thanh toán.' });
        }
        const refundCount = order.statusHistory.filter(s => s.status === 'refund_requested').length;
        const maxRefund = 3;
        const remain = Math.max(0, maxRefund - refundCount);
        if (refundCount > maxRefund) {
            ...
            await order.save();
            await createNotificationForUser(order.user, 'Từ chối hoàn tiền', `Bạn đã vượt quá số lần yêu cầu hoàn tiền cho đơn hàng #${order._id}. Mọi yêu cầu tiếp theo sẽ bị từ chối.`, 'order', `/profile?tab=orders`, { orderId: order._id, remain: 0 });
            return res.status(400).json({ message: 'Bạn đã vượt quá số lần yêu cầu hoàn tiền. Mọi yêu cầu tiếp theo sẽ bị từ chối.' });
        }
        if (order.status === 'refund_requested') {
            return res.status(400).json({ message: 'Đơn hàng đã có yêu cầu hoàn tiền.' });
        }
        ...
        await order.save();
        await createNotificationForUser(order.user, 'Yêu cầu hoàn tiền', `Bạn đã gửi yêu cầu hoàn tiền cho đơn hàng #${order._id}. Bạn còn ${remain} lần yêu cầu hoàn tiền.`, 'order', `/profile?tab=orders`, { orderId: order._id, remain });
        res.json(order);
    } catch (error) {
        res.status(400).json({ message: error.message });
    }
...
const rand = Math.floor(Math.random() * 1000000).toString().padStart(6, '0'); // 6 số ngẫu nhiên
const transID = `${yymmdd}_${rand}`;
...
callback_url: "http://localhost:8000/api/order/zalo-pay/callback",    };
