...
async function fetchAllAddressData() {
    try {
        ...
        // Lấy chi tiết từng tỉnh (bao gồm quận/huyện và phường/xã)
        ...
        for (let i = 0; i < provinces.length; i++) {
            ...
            try {
                ...
                // API trả về wards trực tiếp, không có districts
                // Tạo cấu trúc districts giả lập từ wards
                const districts = [];
                const wardsByDistrict = {};
                
                if (provinceData.wards && provinceData.wards.length > 0) {
                    // Nhóm wards theo district (sử dụng 2 số đầu của ward code)
                    provinceData.wards.forEach(ward => {
                        const districtCode = Math.floor(ward.code / 1000) * 1000; // Làm tròn xuống hàng nghìn
